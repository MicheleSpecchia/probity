...
from pmx.monitoring.artifact import MONITORING_REPORT_ARTIFACT_SCHEMA_VERSION
from pmx.news.normalize import canonicalize_json

MONITORING_REPORT_ARTIFACT_SCHEMA_FILENAME = "monitoring_report_artifact.v1.json"
SCHEMAS_DIR = Path(__file__).resolve().parent / "schemas"


def validate_monitoring_report_artifact(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if schema_version and schema_version != MONITORING_REPORT_ARTIFACT_SCHEMA_VERSION:
        return [
            {
                "code": "schema_version",
                ...
                "reason": (
                    ...
                    f"expected {MONITORING_REPORT_ARTIFACT_SCHEMA_VERSION!r}"
                ),
            }
        ]
...
def _load_artifact_schema() -> dict[str, Any]:
    path = SCHEMAS_DIR / MONITORING_REPORT_ARTIFACT_SCHEMA_FILENAME
    if not path.exists():
