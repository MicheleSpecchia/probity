...
from pmx.decisions.artifact import DECISION_ARTIFACT_SCHEMA_VERSION
from pmx.news.normalize import canonicalize_json

DECISION_ARTIFACT_SCHEMA_FILENAME = "decision_artifact.v1.json"
SCHEMAS_DIR = Path(__file__).resolve().parent / "schemas"


def validate_decision_artifact(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    schema_version = str(canonical.get("decision_schema_version", ""))
    if schema_version and schema_version != DECISION_ARTIFACT_SCHEMA_VERSION:
        return [
            {
                "code": "schema_version",
                "path": "$.decision_schema_version",
                "reason": (
                    f"Unsupported decision_schema_version {schema_version!r}; "
...
def _load_artifact_schema() -> dict[str, Any]:
    path = SCHEMAS_DIR / DECISION_ARTIFACT_SCHEMA_FILENAME
    if not path.exists():
