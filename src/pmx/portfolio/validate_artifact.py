...
from pmx.portfolio.artifact import PORTFOLIO_ARTIFACT_SCHEMA_VERSION

PORTFOLIO_ARTIFACT_SCHEMA_FILENAME = "portfolio_artifact.v1.json"
SCHEMAS_DIR = Path(__file__).resolve().parent / "schemas"


def validate_portfolio_artifact(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if schema_version and schema_version != PORTFOLIO_ARTIFACT_SCHEMA_VERSION:
        return [
            {
                "code": "schema_version",
                ...
                "reason": (
                    ...
                    f"expected {PORTFOLIO_ARTIFACT_SCHEMA_VERSION!r}"
                ),
            }
        ]
...
def _load_artifact_schema() -> dict[str, Any]:
    path = SCHEMAS_DIR / PORTFOLIO_ARTIFACT_SCHEMA_FILENAME
    if not path.exists():
