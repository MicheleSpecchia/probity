...
from pmx.execution.artifact import EXECUTION_ARTIFACT_SCHEMA_VERSION
from pmx.news.normalize import canonicalize_json

EXECUTION_ARTIFACT_SCHEMA_FILENAME = "execution_artifact.v1.json"
SCHEMAS_DIR = Path(__file__).resolve().parent / "schemas"


def validate_execution_artifact(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if schema_version and schema_version != EXECUTION_ARTIFACT_SCHEMA_VERSION:
        return [
            {
                "code": "schema_version",
                ...
                "reason": (
                    ...
                    f"expected {EXECUTION_ARTIFACT_SCHEMA_VERSION!r}"
                ),
            }
        ]
...
def _load_artifact_schema() -> dict[str, Any]:
    path = SCHEMAS_DIR / EXECUTION_ARTIFACT_SCHEMA_FILENAME
    if not path.exists():
