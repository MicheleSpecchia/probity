...
from pmx.performance.artifact import PERFORMANCE_REPORT_ARTIFACT_SCHEMA_VERSION

PERFORMANCE_REPORT_ARTIFACT_SCHEMA_FILENAME = "performance_report_artifact.v1.json"
SCHEMAS_DIR = Path(__file__).resolve().parent / "schemas"


def validate_performance_report_artifact(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if schema_version and schema_version != PERFORMANCE_REPORT_ARTIFACT_SCHEMA_VERSION:
        return [
            {
                "code": "schema_version",
                ...
                "reason": (
                    ...
                    f"expected {PERFORMANCE_REPORT_ARTIFACT_SCHEMA_VERSION!r}"
                ),
            }
        ]
...
def _load_artifact_schema() -> dict[str, Any]:
    path = SCHEMAS_DIR / PERFORMANCE_REPORT_ARTIFACT_SCHEMA_FILENAME
    if not path.exists():
