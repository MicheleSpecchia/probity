...
from pmx.trade_plan.artifact import TRADE_PLAN_ARTIFACT_SCHEMA_VERSION

TRADE_PLAN_ARTIFACT_SCHEMA_FILENAME = "trade_plan_artifact.v1.json"
SCHEMAS_DIR = Path(__file__).resolve().parent / "schemas"


def validate_trade_plan_artifact(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if schema_version and schema_version != TRADE_PLAN_ARTIFACT_SCHEMA_VERSION:
        return [
            {
                "code": "schema_version",
                ...
                "reason": (
                    ...
                    f"expected {TRADE_PLAN_ARTIFACT_SCHEMA_VERSION!r}"
                ),
            }
        ]
...
def _load_artifact_schema() -> dict[str, Any]:
    path = SCHEMAS_DIR / TRADE_PLAN_ARTIFACT_SCHEMA_FILENAME
    if not path.exists():
