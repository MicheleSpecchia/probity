...
from pmx.audit_bundle.artifact import AUDIT_BUNDLE_ARTIFACT_SCHEMA_VERSION
from pmx.news.normalize import canonicalize_json

AUDIT_BUNDLE_ARTIFACT_SCHEMA_FILENAME = "audit_bundle_artifact.v1.json"
SCHEMAS_DIR = Path(__file__).resolve().parent / "schemas"


def validate_audit_bundle_artifact(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if schema_version and schema_version != AUDIT_BUNDLE_ARTIFACT_SCHEMA_VERSION:
        return [
            {
                "code": "schema_version",
                ...
                "reason": (
                    ...
                    f"expected {AUDIT_BUNDLE_ARTIFACT_SCHEMA_VERSION!r}"
                ),
            }
        ]
...
def _load_artifact_schema() -> dict[str, Any]:
    path = SCHEMAS_DIR / AUDIT_BUNDLE_ARTIFACT_SCHEMA_FILENAME
    if not path.exists():
