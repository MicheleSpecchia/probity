...
from pmx.risk.artifact import RISK_ARTIFACT_SCHEMA_VERSION

RISK_ARTIFACT_SCHEMA_FILENAME = "risk_artifact.v1.json"
SCHEMAS_DIR = Path(__file__).resolve().parent / "schemas"


def validate_risk_artifact(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if schema_version and schema_version != RISK_ARTIFACT_SCHEMA_VERSION:
        return [
            {
                "code": "schema_version",
                ...
                "reason": (
                    ...
                    f"expected {RISK_ARTIFACT_SCHEMA_VERSION!r}"
                ),
            }
        ]
...
def _load_artifact_schema() -> dict[str, Any]:
    path = SCHEMAS_DIR / RISK_ARTIFACT_SCHEMA_FILENAME
    if not path.exists():
